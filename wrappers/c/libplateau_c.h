...
/// 2つの関数を生成します。
/// 詳しい説明は類似マクロ DLL_STRING_PTR_ARRAY_FUNC2 とほぼ同じなのでそちらをご覧ください。
/// 違いはポインタの配列を渡すか、与えられたポインタの領域に対してコピーを書き込むかです。
/// 使い分けについてもそちらで説明しています。
#define DLL_STRING_VALUE_ARRAY_FUNC2(FUNC_NAME, TARGET_TYPE, ARRAY_LENGTH_GETTER, FOR_RANGE, STRING_GETTER, ...) \
    ...
    DLL_VALUE_FUNC(FUNC_NAME ## _count,\
                    ...
                    , __VA_ARGS__)                                                                           \
    \
    /* 各文字列のコピーを与えられたメモリ領域に書き込み、書く文字列のバイト数を書き込む関数です。*/ \
    LIBPLATEAU_C_EXPORT APIResult LIBPLATEAU_C_API FUNC_NAME(\
            const TARGET_TYPE *const handle,\
            char** out_strs,\
            int* out_str_lengths\
        ...
        API_TRY {\
            ...
            for (FOR_RANGE) {\
                ...
                auto& str_val = (STRING_GETTER);                                                                                                 \
                auto chars = str_val.c_str();\
                auto len = (dll_str_size_t) (str_val.length() + 1); /* +1はnull終端文字列の分 */\
                strncpy(out_strs[i], chars, len);\
                out_strs[i][len] = '\0'; /* 最後はnull終端文字*/\
                out_str_lengths[i] = len;\
                i++;\
            }\
            return APIResult::Success;\
        }\
