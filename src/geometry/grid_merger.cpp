...
CityObjectWithImportID::CityObjectWithImportID(const CityObject *const city_object, int primary_import_id, int secondary_import_id) :
...
namespace{
     ...
     */
    int getGridId(const Envelope& city_envelope, const TVec3d position, const int grid_num_x, const int grid_num_y){
     ...
     * また ImportID を割り振ります。
     */
    GridIDToObjsMap classifyCityObjsToGrid(const ConstCityObjects& city_objs, const Envelope& city_envelope, int grid_num_x, int grid_num_y){
...
GridMergeResult GridMerger::gridMerge(const CityModel &city_model, const MeshExtractOptions &options, unsigned lod) {
    // city_model に含まれる 主要地物 をグリッドに分類します。
    const auto& primary_city_objs = city_model.getAllCityObjectsOfType(PrimaryCityObjectTypes::getPrimaryTypeMask());
    const auto& city_envelope = city_model.getEnvelope();
    auto grid_id_to_objs_map = classifyCityObjsToGrid(primary_city_objs, city_envelope, options.grid_count_of_side, options.grid_count_of_side);
    ...
    for(const auto& grid_objs_pair : grid_id_to_objs_map){
        ...
        for(const auto& obj : grid_objs){
            ...
            for(unsigned check_lod = lod + 1; check_lod <= GeometryUtils::max_lod_in_specification_; ++check_lod){
                bool polygon_in_lod_exists = (check_lod == lod) || (findFirstPolygon(obj.getCityObject(), check_lod) != nullptr);
    ...
    for(const auto& group : group_id_to_objs_map){
        ...
        for(const auto& city_obj : group_objs){
            ...
            for(const auto& poly : polygons){
                ...
                const auto uv_2 = TVec2f((float)(city_obj.getPrimaryImportID()) + (float)0.25, 0); // +0.25 する理由は、floatの誤差があっても四捨五入しても切り捨てても望みのint値を得られるためです。
