...
namespace plateau::meshWriter {
    ...
    protected:
        ...
        fs::path expected_output_gltf_ = fs::u8path(output_directory_) / fs::u8path(basename_ + ".gltf");
        fs::path expected_output_bin_ = fs::u8path(output_directory_) / fs::u8path(basename_ + ".bin");
