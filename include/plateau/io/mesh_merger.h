...
class LIBPLATEAU_EXPORT MeshMerger {
    using GridMergeResult = std::shared_ptr<std::vector<PolygonWithUV2*>>;
public:
    /**
     * cityModel の範囲をグリッド状に分割して、グリッド内のメッシュを結合します。
     * 結果は getLastGridMergeResult で取得できます。
     */
    void gridMerge(const CityModel *cityModel, CityObject::CityObjectsType targetTypeMask, int gridNumX, int gridNumY, std::shared_ptr<PlateauDllLogger> logger);
